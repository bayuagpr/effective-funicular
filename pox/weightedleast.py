...
class iplb(object):
    ...
    def _do_expire(self):
        ...
        for key, val in memory.items():
            ...
            if ip in self.live_servers and val.is_expired:
                ...
                self.value[ip] = (self.total_connection[ip] * 10000) / self.weight[ip]
    ...
    def _pick_server(self, key, inport):
        ...
        ipserver = self.value.keys()[0]
        least_value = self.value[ipserver]

        # if len(self.total_connection) == 0:
        #    return self.live_servers.keys()[0]
        # ipserver = self.total_connection.keys()[0]
        # totalconns = self.total_connection[ipserver]
        self.log.debug(self.total_connection)
        for x in self.value:
            if self.value[x] < least_value:
                ipserver = x
                least_value = self.value[ipserver]
    ...
    def _handle_PacketIn(self, event):
        ...
        # Incoming packet from client
            ...
            if entry is None or entry.server not in self.live_servers:
                ...
                self.value[server] = (self.total_connection[server] * 10000) / self.weight[server]
            # Update timestamp
            entry.refresh()
